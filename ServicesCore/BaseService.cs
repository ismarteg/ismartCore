...
namespace ServicesCore
{
    ...
    where TDto : class
    {
        ...
        public virtual SrvResponse GetAll()
        {
            try
            {
                var data = _TbRepository.GetAll(x => !x.GetIsDeleted()).ToList();
        ...
        public SrvResponse GetList(int PageIndex = 1, int PageSize = 50)
        {
            try
            {
                var data = _TbRepository.GetPage(PageIndex, PageSize, out int totalCount, x => !x.GetIsDeleted()).ToList();
                var result = data.MapList<TDto>();
                return _response.Success(result);
            }
            catch (Exception ex)
